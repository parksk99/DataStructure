template<typename T>
ChainNode<T>* CircularChain<T>::asl;
template<typename T>
...
CircularChain<T>::CircularChain(const CircularChain<T>& c){
	~CircularChain();
	CircularChain<T>::ChainIterator i;
...
template<typename T>
CircularChain<T>::ChainIterator::ChainIterator(ChainNode<T>* start)
	: current(start){}

	template<typename T>
	T& CircularChain<T>::ChainIterator::operator*() const{
		return current->data;
	}
...
template<typename T>
bool CircularChain<T>::ChainIterator::operator!=(ChainNode<T>* x){
	return current!=x;
}


template<typename T>
void CircularChain<T>::Insert(T newData){
	if(first){
		last->link = GetNode();
		last = last->link;
		last->data = newData;
...
template<typename T>
void CircularChain<T>::Delete(int i){
	if(i>=size) {
...
template<typename T>
void CircularChain<T>::ReturnNode(ChainNode<T>* x){
	x->link = asl;
	asl = x;
}

template<typename T>
T CircularChain<T>::Get(int i){
